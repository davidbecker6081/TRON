...
let battleSound = new SoundFx(document.getElementById('battle-sound'))
let gameplaySound = new SoundFx(document.getElementById('gameplay-sound'))
let startSound = new SoundFx(document.getElementById('start-sound'))
let gameoverSound = new SoundFx(document.getElementById('gameover-sound'))
let battleExplosion = new SoundFx(document.getElementById('battle-explosion'))
...
function countDownStart() {
  if (countDownNum > 0 && resetGamePress === false) {
    ...
    countDownNum--
    setTimeout(countDownStart, 2000)
...
function displayNewGameText() {
  ...
  contextOverlay.strokeText('To Enter The Grid', 110, canvasOverlay.height / 2 + 30)
}
...
function gameOverText(winner) {
  gameplaySound.stopFX()
  gameoverSound.playFX()
  setTimeout(() => {
    gameoverSound.stopFX()
  }, 9000)
  clearOverlay()
  contextOverlay.strokeStyle = 'cyan'
  contextOverlay.font = '50px Tr2n'
  contextOverlay.strokeText('GAME OVER', canvasOverlay.width / 2 - 160, canvasOverlay.height / 2 - 72)
  contextOverlay.strokeText(`${winner} Wins!`, canvasOverlay.width / 2 - 200, canvasOverlay.height / 2 + 30)
  $resetBtn.addClass('blink')
}
...
function pauseGame() {
  ...
  pauseBtnText()
  showCanvasOverlay()
  $pauseBtn.addClass('blink')
  if (isGamePaused === false) {
    $pauseBtn.removeClass('blink')
    hideCanvasOverlay()
    gameLoop()
  }
}

function pauseBtnText() {
  contextOverlay.strokeStyle = 'cyan'
  contextOverlay.font = '80px Tr2n'
  contextOverlay.strokeText('Paused', 170, canvasOverlay.height / 2 - 20)
}
...
function resetBtns() {
  ...
  $normalBtn.attr('disabled', false)
  $hardBtn.attr('disabled', false)
  $pauseBtn.removeClass('blink')
  $resetBtn.removeClass('blink')
  $easyBtn.show()
  $normalBtn.show()
  $hardBtn.show()
}
...
function roundEnd() {
  battleExplosion.playFX()
  setTimeout(() => {
    battleExplosion.stopFX()
  }, 1000)
  countdownFinish = true
  isGamePaused = true
  stopPlayers()
...
function roundWinnerText(winner) {
  ...
  contextOverlay.strokeStyle = 'cyan'
  contextOverlay.font = '40px Tr2n'
  contextOverlay.strokeText(winner, 230, canvasOverlay.height / 2 - 72)
  contextOverlay.strokeText(`Wins Round`, canvasOverlay.width / 2 - 130, canvasOverlay.height / 2 + 30)
}
